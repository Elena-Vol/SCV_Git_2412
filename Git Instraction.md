...
```
6. ДЛя переноса информации с удаленного репозитория на локальный используем команду:
```
git pull
```
...
3. Долее нужно создавшуюся копию этого репозитария, но уже в нашем аккаунте скопировать на локальный репозитарий, команда:
```
 git clone <URL - адрес репозитория в сети>
```
4. Нужно сменить директорию, команда:
```
cd .. - переносит на один шаг назад
или 
cd <наименование папки - перенесенного репозитория> - переносит в нужную нам директорию
```
5. Создаем новую ветку, куда будем вносить все наши изменения. И переходим в эту ветку

6. Проверяем, что мы стоим в новой ветке, вносим изменения: можно добавить/создать или скопировать свой файл. Сохраняем эти внесенный изменения и проверяем, что в нашей ветке все сохранилось
7. Переносим изменения в наш удвленный репозитарий, не забыв предвврительно созлав в удаленном репозиротии ветку, которую создали в локальном см. п. 16

8. В удаленном репозитории на Github обновляем страницу, заходим в нужную ветку (новую, которую создали в длокальном репозитории)
9. Для передачи нашей информацции в чужой репозитарий нужно воспользоваться кнопкой `pull request`, если она не появилась как отдельное окно, то в верхнем меню она точно есть)). Нажимаем и  второй раз выбираем кнопку `new pull request`, далее
    1. проверяем куда отправим репозитарий, 
    2. в нашем репозитории выбираем нашу новую ветку (где обновленные коммиты)
    3. Guthub проверяет возможно ли слияние - *Able to merge*
    4. Нажимаем кнопку `cgeate pull request`
    5. Хорошим тоном будет кратко описать автору репозитария в комментарии о чем ваше дополнение
